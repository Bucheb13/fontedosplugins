...
/* =========================
   PAGE
========================= */
export default function PaginaAdminProgramas() {
  ...
  async function carregar() {
    ...
    const res = await fetch("/api/admin/programas/listar", {
  ...
  async function criar() {
    ...
    const res = await fetch("/api/admin/programas/criar", {
      method: "POST",
      headers: { "x-senha-admin": senhaAdmin },
      body: fd,
    });
    ...
    if (!res.ok) return setMensagem(json?.erro ?? "Erro ao criar programa.");

    if (json.programa) setItens((prev) => [json.programa, ...prev]);
  ...
  /* =========================
     EXCLUIR
  ========================== */
  async function excluir(slugPrograma: string) {
    if (!senhaAdmin) return;
    const ok = confirm(`Excluir "${slugPrograma}"?`);
    if (!ok) return;

    setExcluindo(slugPrograma);

    const res = await fetch("/api/admin/programas/deletar", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-senha-admin": senhaAdmin,
      },
      body: JSON.stringify({ slug: slugPrograma }),
    });
    ...
    setItens((prev) => prev.filter((p) => p.slug !== slugPrograma));
    setMensagem("Programa excluído.");
  }
