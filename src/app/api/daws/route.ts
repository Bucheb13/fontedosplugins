...
export async function GET(req: Request) {
  ...
  if (slug) {
    ...
    if (!data) return NextResponse.json({ erro: "DAW não encontrado." }, { status: 404 });

    return NextResponse.json({ daw: data as DawsPublico });
  }
  ...
  return NextResponse.json({ daws: (data ?? []) as DawsPublico[] });
}
