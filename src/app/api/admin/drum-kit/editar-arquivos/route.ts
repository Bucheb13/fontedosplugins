...
export async function POST(req: Request) {
  ...
  const { data: drumKitAtual, error: erroBusca } = await supabase
    .from("drum_kits")
  ...
  if (erroBusca) {
    return jsonErro("Erro ao buscar Drum-Kit atual.", 500);
  }
  
  if (!drumKitAtual) {
    return jsonErro("Drum-Kit não encontrado.", 404);
  }
...
if (capa instanceof File) {
  ...
  if (drumKitAtual.imagem_capa_url) {
    const base = publicBase.replace(/\/$/, "");

    if (drumKitAtual.imagem_capa_url.startsWith(base)) {
      const keyAntiga = drumKitAtual.imagem_capa_url.replace(`${base}/`, "");
