...
export async function POST(req: Request) {
  ...
  // 🔹 Buscar drum-kit
  const { data: drumKit, error: erroBusca } = await supabase
    .from("drum_kits")
  ...
  // 🔹 Deletar TORRENT
  if (drumKit.r2_chave_arquivo) {
    await r2.send(
      new DeleteObjectCommand({
        Bucket: bucket,
        Key: drumKit.r2_chave_arquivo,
      })
    );
  }

  // 🔹 Deletar CAPA
  if (drumKit.imagem_capa_url) {
    const base = process.env.NEXT_PUBLIC_R2_PUBLIC_BASE_URL?.replace(/\/$/, "");
    if (base && drumKit.imagem_capa_url.startsWith(base)) {
      const key = drumKit.imagem_capa_url.replace(`${base}/`, "");
