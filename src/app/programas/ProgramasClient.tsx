...
export default function PaginaPrograma() {
  ...
  const buscarProgramas = async (query: string) => {
    setCarregando(true);
    setErro("");
    try {
      const url = new URL("/api/programas", location.origin);
      ...
      console.error(err);
      setErro("Erro ao carregar Programas");
      setProgramas([]);
    } finally {
      setCarregando(false);
    }
  ...
  useEffect(() => {
    buscarProgramas(debouncedPesquisa);
  }, [debouncedPesquisa]);
        ...
        {programas.map((programa) => (
          ...
          >
            ...
            <div className="flex flex-col gap-2 p-5">
              <h2 className="text-xl leading-tight text-white">{programa.nome}</h2>
              {programa.subtitulo && (
                <p className="text-sm text-white/70 line-clamp-3">{programa.subtitulo}</p>
              )}
